var JEKYLLPLUS_REPO = 'vnium/jekyll-template/gh-pages';
var JEKYLLPLUS_PATH = '';
var JEKYLLPLUS_URL = 'http://localhost:3000';

document.addEventListener('DOMContentLoaded', function() {
  var JEKYLLPLUS_MENU = (window.JEKYLLPLUS_MENU === undefined) ?
    [{
      label: 'Create a page',
      path: 'new?collection=pages'
    }, {
      label: 'Create a post',
      path: 'new?collection=posts'
    }] :
    window.JEKYLLPLUS_MENU;
  var JEKYLLPLUS_PREFIX = JEKYLLPLUS_URL + '/' + JEKYLLPLUS_REPO + '/';

  if ((window.location.href.indexOf('?admin=true') != -1) || (window.location.href.indexOf('&jekyllplus=true') != -1)) {
